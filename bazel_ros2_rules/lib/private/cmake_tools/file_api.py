        ...
        if "link" in target_data:
            ...
            for fragment in link["commandFragments"]:
                if fragment["role"] == "flags":
                    ...
                    # In the generated JSON a library can be in quotes,
                    # in order to escape some characters. This escaping
                    # creates problem down the line, "/usr/lib/libfoo.so"
                    # is *not* a path. We remove the escaping here so they
