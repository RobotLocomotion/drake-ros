...
def collect_ament_python_package_direct_properties(name, metadata, dependencies, cache):
    ...
    if name not in ament_python_cache:
        ament_python_cache[name] = collect_ament_python_package_properties(
            name, metadata
        )
        ...
        for dependency_name, dependency_metadata in dependencies.items():
            ...
            if "cc" in dependency_metadata.get("langs", []):
                if dependency_metadata.get("build_type") == "ament_cmake":
                    ...
                    dependency_libraries.extend(dependency_properties.link_libraries)
            if "py" in dependency_metadata.get("langs", []):
                ...
                if dependency_properties.cc_libraries:
                    dependency_libraries.extend(dependency_properties.cc_libraries)
            # Remove duplicates maintaining order
            properties.cc_libraries = tuple(
                [
                    lib
                    for lib in properties.cc_libraries
