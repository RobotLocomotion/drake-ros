...
def configure_package_cc_library(name, metadata, properties, dependencies, sandbox):
    ...
    for link_directory in properties.link_directories:
        ...
        if not link_directory:
            continue
        linkopts += ["-L" + link_directory, "-Wl,-rpath " + link_directory]
    deps = [
...
def configure_package_py_library(name, metadata, properties, dependencies, sandbox):
    ...
    if properties.cc_extensions or properties.cc_libraries:
        ...
        if properties.cc_libraries:
            ...
            data.extend(cc_deps)
        # Prepare runfiles to support dynamic loading
        cc_extensions = [sandbox(extension) for extension in properties.cc_extensions]
    ...
    return (target_name, load_resource(template), to_starlark_string_dict(config))
...
def configure_package_c_library_alias(name, metadata):
    ...
    return (
        ...
        load_resource("templates/package_alias.bazel.tpl"),
        to_starlark_string_dict(
            {"name": target_name, "actual": cc_label(name, metadata)}
        ),
    )


def configure_executable_imports(executables, dependencies, sandbox, prefix=None):
    deps = []
    common_data = []
    for dependency_name, dependency_metadata in dependencies.items():
...
def configure_package_executable_imports(name, metadata, dependencies, sandbox):
    ...
    yield from configure_executable_imports(
        ...
        dependencies,
        sandbox,
        prefix=label_name(name, metadata),
    )
