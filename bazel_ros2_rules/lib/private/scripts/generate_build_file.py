...
"""
Scrapes ROS 2 workspaces and exposes their artifacts through a Bazel
local repository.

This script generates:

- a BUILD.bazel file, with targets for all C/C++ libraries, Python
...
from ros2bzl.scraping.ament_cmake import (
    collect_ament_cmake_package_direct_properties,
    precache_ament_cmake_properties,
)
from ros2bzl.scraping.ament_python import (
    collect_ament_python_package_direct_properties,
)
from ros2bzl.scraping.python import (
    PackageNotFoundError,
    collect_python_package_properties,
)
from ros2bzl.templates import (
    configure_executable_imports,
    configure_package_c_library_alias,
    configure_package_cc_library,
    configure_package_executable_imports,
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Number of CMake jobs to use during package configuration scraping"
        ),
    )
    parser.add_argument(
        "-d",
    ...
    parser.add_argument("repository_name", help="Bazel repository name")
    args = parser.parse_args()

    args.sandbox = sandboxing.make_path_mapping(
        name=args.repository_name,
        mapping=dict(entry.split(":") for entry in args.sandbox_mappings),
    )
...
def write_build_file(fd, repo_name, distro, sandbox, cache):
    rmw_implementation_packages = {
        name: metadata
        for name, metadata in distro["packages"].items()
    ...
    for name in toposort.toposort_flatten(dependency_graph):
        ...
        direct_dependency_names = dependency_graph[name].intersection(
        ...
        if "share_directory" in metadata:
            _, template, config = configure_package_share_filegroup(
                name, metadata, sandbox
            )
            fd.write(interpolate(template, config) + "\n")

        if "rosidl_interface_packages" in metadata.get("groups", []):
            _, template, config = configure_package_interfaces_filegroup(
                name, metadata, sandbox
            )
            fd.write(interpolate(template, config) + "\n")
        ...
        if build_type in ("ament_cmake", "cmake"):
            ...
            if "rosidl_interface_packages" in metadata.get("groups", []):
                ...
                _, template, config = configure_package_c_library_alias(
                    name, metadata
                )
                fd.write(interpolate(template, config) + "\n")
...
def main():
    ...
    ament_cmake_cache = precache_ament_cmake_properties(
