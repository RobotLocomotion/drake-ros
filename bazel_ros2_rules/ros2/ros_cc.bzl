...
def ros_cc_binary(
    ...
    no subprocess indirection, with the following caveats:
      ...
      * It does not support programs where the main() function is defined in a
        ...
        Note that unit test frameworks in particular often a library main().
    """
    ...
    # Declare an unshimmed, manual binary. For the "reeexec" case, this will be
    ...
    if shim == "reexec":
        shim_kwargs = dict(kwargs)
        shim_tags = shim_kwags.pop("tags", [])
        ...
        cc_library_rule(
            ...
            srcs = [main_cc],
            linkstatic = True,
            deps = ["@bazel_ros2_rules//ros2:dload_shim"],
            tags = main_tags,
            **main_kwargs
        )
...
def ros_cc_test(
    ...
    This wrapper does not support programs where the main() function is defined
    in a cc_library. Always pass the main() function definition in `srcs`.
    """
