...
cc_library(
    name = "headers",
    hdrs = glob(
        include = ["_include/numpy/**/*.*"],
        exclude_directories = 1,
    ),
    deps = ["@@{}//:headers"],
    includes = ["_include/numpy"],
)
...
def _impl(repo_ctx):
    ...
    for cflag in cflags:
        if not cflag.startswith("-I"):
            continue
        include = cflag[2:]
        sandboxed_include = "_include/cpython/{}".format(
            include.replace("/", "_"),
        )
    ...
    repo_ctx.file(
        "BUILD.bazel",
        content = ROOT_BUILD_FILE_TEMPLATE.format(
            includes,
            linkopts,
        ),
        executable = False,
    ...
    repo_ctx.symlink(numpy_include_dir, "numpy/_include/numpy")

    repo_ctx.file(
        "numpy/BUILD.bazel",
        content = NUMPY_BUILD_FILE_TEMPLATE.format(
            repo_ctx.name,
            repo_ctx.name,
        ),
        executable = False,
...
local_python_repository = repository_rule(
