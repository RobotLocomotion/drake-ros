...
def test_nominal_case(drake_ros_fixture):
    ...
    system_pub_out = builder.AddSystem(
    ...
    system_sub_in = builder.AddSystem(
    ...
    builder.Connect(
        system_sub_in.get_output_port(0), system_pub_out.get_input_port(0)
    )
    ...
    _direct_sub_out = direct_ros_node.create_subscription(
    ...
    for i in range(1, pub_sub_rounds + 1):
        ...
        # just yet.
        rclpy.spin_once(direct_ros_node, timeout_sec=0.0)
        simulator.AdvanceTo(simulator_context.get_time() + publish_period / 2.0)
